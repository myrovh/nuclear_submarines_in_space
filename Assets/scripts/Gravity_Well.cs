...
public class Gravity_Well : MonoBehaviour {
    ...
    void OnTriggerEnter(Collider other_collider)
    {
        ...
        {
            objects_in_range.Add(other_collider.GetComponent<Rigidbody>()); // Add check to debug log if no rigidbody exists
        }
    }
    ...
    void OnTriggerExit(Collider other_collider)
    {
        if (other_collider.tag == target_tag)
        {
            objects_in_range.Remove(other_collider.GetComponent<Rigidbody>()); // Add check to debug log if no rigidbody exists
        }
    }
    ...
    void FixedUpdate()
    {
        ...
        {
            force_muliplier = (-gravity_strength / Mathf.Pow(Mathf.Max(Vector3.Distance(transform.position, a.position), 1f), gravity_strength_exponent));
